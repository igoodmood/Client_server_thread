...
public class Client  {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < clients.length; i++) {
            clients[i] = new Client(serverAddress, portNumber, "thread" + i);
            clients[i].start();
        }
...
class Generate extends Thread {
    ...
    private ArrayList<String> send_task = new ArrayList<>();
    private Client[] clientser = stopWatch.getClients();

    @Override
    public void run() {
        sdf = new SimpleDateFormat("HH:mm:ss");
        for (Client client : clientser) {
            stopWatch.addPlainTexter(" > " + sdf.format(new Date()) + " получен и обработан запрос пользователя " + client.getUsername() + ".");
            send_task.add(String.valueOf(new Random().nextInt(200) + 1));
        }
        for (int i = 0; i < send_task.size(); i++) {
            clientser[i].sendMessage(new ChatMessage(ChatMessage.MESSAGE, send_task.get(i)));
        }
    }
}